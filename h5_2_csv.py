...
def h5_to_csv():
    ...
    for n in range(len(data['imgname'].value)):
        ...
        w_box = (xmax - xmin) / 2 * scale_w
        h_box = (ymax - ymin) / 2 * scale_h
...
def get_image_name():
    data = h5py.File('./valid.h5')
    name = data['imgname'].value[:].astype(str)
    name_new = []
    for n in name:
        if n not in name_new:
            name_new.append(n)
    name_new = np.array(name_new, dtype=str)
    np.save('./val_name.npy', name_new)
    print(len(name_new))
# get_image_name()
